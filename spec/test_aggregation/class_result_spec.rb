describe TestAggregation::ClassResult do
  ...
  describe '#results_hash' do
    it 'sums individual step results counts form test_steps#results_hash' do
      ...
      expect(subject).to receive(:test_steps).and_return(
        [test_step1, test_step2]
      )
      expect(subject.results_hash).to eq('passed' => 1,
                                         'failed' => 1,
                                         'created' => 5)
    end
  end
  ...
  describe '#parse' do
    it 'raise exception when class_name is not defined' do
      expect do
        subject.parse({})
      end.to raise_error(/Class name not defined/)
    end

    it 'raise exception when class_name differs' do
      subject.parse('class_name' => 'CN1', 'position' => 1, 'name' => 'N')
      expect do
        subject.parse('class_name' => 'CN2', 'position' => 1, 'name' => 'N')
      end.to raise_error(/Class name mishmash/)
    end

    it 'raise exception when position is not defined' do
      expect do
    ...
    it 'creates instance of StepResult on passed position' do
      subject.parse('class_name' => 'CN1', 'position' => 3, 'name' => 'N')
